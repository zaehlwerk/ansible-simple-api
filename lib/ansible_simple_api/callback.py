...
class ResultCallback(CallbackBase):
    ...
    def default_action(self, result, state):
        try:
            host_results = self.results[result._host.name]
        except KeyError:
            host_results = []
            self.results[result._host.name] = host_results
        host_results.append((result.task_name, state, result._result))

    def v2_runner_on_failed(self, result, ignore_errors=False):
        self.default_action(result, "failed")

    def v2_runner_on_ok(self, result, **kwargs):
        self.default_action(result, "ok")

    def v2_runner_on_skipped(self, result):
        self.default_action(result, "unreachable")
